...
| `components/` | 39 files, 17,634 lines | 18 files, 7,296 lines | 1 file, 137 lines |
| `collab/` | 13 files, 11,961 lines | 16 files, 13,178 lines | none |
| `pro/` | 20 files, 7,365 lines | 10 files, 1,576 lines | 2 files, 1,194 lines |
...
| `styles/` | none | 1 file, 151 lines | 63 files, 29,877 lines |

Plus 37 `.ts`/`.js` files at the top level of `src/`, 10,989 lines all told, of which 12 are tests and 3 are ambient declarations. `main.ts` is 1,251 of that.
<!-- web-src-dirs:end -->
...
| 4,163 | `views/tool.ts` | partial — the undo/redo model (coalescing, the byte-carrying filter, the cap, the redo chain) is extracted to `tool-history.ts` and covered by `tool-history.test.ts`; the rest of `mountTool` is not. |
