...
<!-- web-src-dirs:start -->
Roughly 349,000 lines of TypeScript, tests included, and 34,000 lines of CSS.

| Directory | Source | Tests | CSS |
|---|---|---|---|
| `views/` | 76 files, 84,211 lines | 60 files, 27,532 lines | none |
| `lib/` | 250 files, 67,604 lines | 147 files, 35,093 lines | 7 files, 1,060 lines |
| `bridge/` | 81 files, 36,590 lines | 60 files, 15,417 lines | none |
| `components/` | 40 files, 18,108 lines | 18 files, 7,396 lines | 1 file, 137 lines |
| `collab/` | 18 files, 12,870 lines | 20 files, 13,682 lines | none |
| `pro/` | 20 files, 7,426 lines | 10 files, 1,628 lines | 2 files, 1,194 lines |
| `org/` | 14 files, 5,370 lines | 12 files, 3,642 lines | none |
| `catalog/` | 2 files, 673 lines | 1 file, 86 lines | none |
| `ext/` | 2 files, 136 lines | 1 file, 86 lines | none |
| `styles/` | none | 1 file, 151 lines | 67 files, 31,967 lines |

Plus 37 `.ts`/`.js` files at the top level of `src/`, 11,182 lines all told, of which 12 are tests and 3 are ambient declarations. `main.ts` is 1,317 of that.
<!-- web-src-dirs:end -->
...
| 3,520 | `views/picker.ts` | partial — the format and embeddability rules are extracted to `picker-formats.ts` and covered by `picker-formats.test.ts`, plus `picker-initial-tab.test.ts`; the 3,000-line panel body is not. |
| 3,382 | `views/color-lab.ts` | yes |
| 3,131 | `views/tool-actions.ts` | yes |
| 2,996 | `views/tool-inputs.ts` | none |
| 2,864 | `views/profile.ts` | none |
| 2,844 | `views/gallery.ts` | none |
| 2,840 | `views/valid.ts` | `valid-verdict.test.ts` only |
| 2,804 | `views/start.ts` | yes |
| 2,670 | `views/deck-editor.ts` | yes |
| 2,304 | `bridge/sequence-render.ts` | yes |
| 2,131 | `views/design-import.ts` | **none** |
| 2,062 | `views/projects.ts` | none |
