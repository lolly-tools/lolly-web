...
<!-- web-src-dirs:start -->
Roughly 420,000 lines of TypeScript, tests included, and 39,000 lines of CSS.

| Directory | Source | Tests | CSS |
|---|---|---|---|
| `views/` | 94 files, 100,859 lines | 89 files, 34,423 lines | none |
| `lib/` | 355 files, 92,672 lines | 216 files, 47,333 lines | 7 files, 1,098 lines |
| `bridge/` | 88 files, 39,643 lines | 70 files, 17,246 lines | none |
| `components/` | 42 files, 19,093 lines | 23 files, 8,123 lines | 1 file, 138 lines |
| `collab/` | 18 files, 12,959 lines | 20 files, 13,682 lines | none |
| `pro/` | 20 files, 7,873 lines | 10 files, 1,666 lines | 2 files, 1,210 lines |
...
| `styles/` | none | 3 files, 761 lines | 74 files, 36,982 lines |

Plus 39 `.ts`/`.js` files at the top level of `src/`, 12,895 lines all told, of which 14 are tests and 3 are ambient declarations. `main.ts` is 1,865 of that.
<!-- web-src-dirs:end -->
...
| 5,098 | `lib/brand-editor.ts` | none |
| 5,024 | `views/tool.ts` | partial - the undo/redo model (coalescing, the byte-carrying filter, the cap, the redo chain) is extracted to `tool-history.ts` and covered by `tool-history.test.ts`; the rest of `mountTool` is not. |
| 4,090 | `views/picker.ts` | partial - the format and embeddability rules are extracted to `picker-formats.ts` and covered by `picker-formats.test.ts`, plus `picker-initial-tab.test.ts`; the 3,000-line panel body is not. |
| 3,915 | `views/valid.ts` | `valid-verdict.test.ts` only |
| 3,713 | `views/tool-actions.ts` | yes |
| 3,656 | `views/color-lab.ts` | yes |
| 3,252 | `views/projects.ts` | none |
| 3,251 | `views/profile.ts` | none |
| 3,203 | `views/gallery.ts` | none |
| 3,098 | `views/tool-inputs.ts` | none |
| 2,898 | `views/start.ts` | yes |
| 2,675 | `views/deck-editor.ts` | yes |
| 2,639 | `bridge/sequence-render.ts` | yes |
| 2,322 | `views/design-import.ts` | **none** |
| 2,062 | `components/collab-ceremony.ts` | yes |
| 2,026 | `lib/clip-thumbs.ts` | yes |
<!-- web-src-largest:end -->
