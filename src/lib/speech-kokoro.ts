...
/**
 * Kokoro speech synthesis — the PURE half. Everything here is plain math and
 * string work with no transformers.js, no phonemizer wasm and no DOM, so it
 * runs (and is tested) in Node while the worker
 * (lib/speech-kokoro-worker.ts) supplies the model, the tokenizer and the
 * eSpeak phonemizer around it. Same split as the engine's analysePcm: the
 * heavy runtime is injectable, the bookkeeping is testable.
 *
...
import type { SpeechVoiceInfo, SpeechWordTiming } from '@lolly-tools/core/host-v1';

export const KOKORO_SAMPLE_RATE = 24000;
export const KOKORO_STYLE_DIM = 256;
/** transformers.js model id under env.localModelPath ('/models/') — /models/kokoro/. */
