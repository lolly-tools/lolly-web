...
/**
 * Speech-privacy DRIFT GUARD — a static source-scan of the Kokoro worker, in
 * the a11y-prefs-contract.test.ts mould. The whole "no text or bytes ever
 * leave the device" promise rides on three assignments in
 * lib/speech-kokoro-worker.ts, and no runtime unit test can see them (the
 * worker needs transformers.js and a wasm runtime to even import). A
 * transformers.js upgrade or an innocent refactor must not silently
 * re-enable huggingface.co fetches.
 *
...
const src = readFileSync(join(LIB_DIR, 'speech-kokoro-worker.ts'), 'utf8');

describe('speech worker privacy pins', () => {
