...
html[data-a11y-motion="reduce"] button.neuro-dock-grip { transition: none; }
...
/* Same rules for the app's own preference (data-a11y-motion, lib/a11y-prefs.ts);
   the entrance is also skipped in JS below, this covers a dock already animating
   when the pref is flipped. */
html[data-a11y-motion="reduce"] .neuro-dock { transition: none; }
html[data-a11y-motion="reduce"] .neuro-dock.is-entering { animation: none; }
...
export function showNeuroDock(host: NeurospicyHost, opts: boolean | NeuroDockShowOpts = false): void {
  ...
  // Asked here rather than at import time: the pref can be flipped at any point,
  // and at module load the profile hasn't been hydrated onto <html> yet.
  if (animateIn && (wasHidden || first) && !prefersReducedMotion()) {
