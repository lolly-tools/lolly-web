...
import {
  ...
  embedC2pa, C2PA_FORMATS, packTiff, ENGINE_VERSION,
...
function pureRotationDeg(transform: string | null | undefined): number {
...
function detectUnsupportedCss(el: Element, s: CSSStyleDeclaration, vectorCaps?: { blend?: boolean; clipBasicShapes?: boolean; dropShadow?: boolean }): string | null {
...
function parseCssColorFull(cssColor: string | null | undefined): Rgba | null {
...
async function rasterizeNodeToDataUrl(el: HTMLElement, pxW: number, pxH: number, bg?: string): Promise<string | null> {
  const r = el.getBoundingClientRect();
...
function parseCssColor(cssColor: string | null | undefined): Rgb | null {
  if (!cssColor || cssColor === 'transparent') return null;
  const m = cssColor.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*([\d.]+))?\s*\)/);
  if (!m) return null;
...
function parseCssLen(val: string | null | undefined, refPx: number): number {
...
function resolveRadii(style: CSSStyleDeclaration, w: number, h: number): { radii: CornerRadii; uniform: CornerPair | null } {
  const radii = cornerRadii({
...
function objectPositionFractions(val: string | null | undefined): [number, number] {
...
async function inlineBlobUrlsInEl(el: Element): Promise<void> {
