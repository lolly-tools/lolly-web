...
interface StageNav { reset(): void; isZoomed(): boolean; sync(): void; destroy(): void; }
...
export async function mountTool(viewEl: ViewEl, host: WebToolHost, toolId: string, urlParams: string | null | undefined): Promise<void> {
  ...
  viewEl.innerHTML = `
    ...
    <div class="tool-layout${chromeless ? ' is-editor' : ''}${documentLayout ? ' is-document' : ''}" id="tool-layout"${documentLayout ? ' data-theme="light"' : ''} data-sidebar="${noAside ? 'hidden' : (sidebarOpen ? 'open' : 'closed')}">
      ${showAside ? `
...
function setupStageNav(stageEl: HTMLElement, outerEl: HTMLElement, canvasEl: HTMLElement | null, nativeW: number, onFit: (() => void) | null | undefined, themeToggle?: HTMLElement, soundToggle?: HTMLElement): StageNav {
